// Strike Pulse FINAL unified bundle 20260926
(() => {
  'use strict';

  // Separate guard so an older WordPress-cached loader cannot block this version.
  if (window.__SP_MARKET_FINAL_20260926__) return;
  window.__SP_MARKET_FINAL_20260926__ = true;
...
function render(data) {
  ...
  function init() {
    ...
    console.log('[StrikePulse] MARKET FINAL BUNDLE READY');
  }
...
// Advanced chart engine, bundled locally — no second WordPress script.
function spStartAdvanced(){
...
function clearChart(){if(state.chart){state.chart.remove();state.chart=null}if(state.volume){state.volume.remove();state.volume=null}state.overlays=[];state.priceSeries=null;message('sp3-price-chart','');message('sp3-volume-chart','')}
...
if(!state.lib){box.innerHTML='<div class="sp3-chart-placeholder">Loading interactive chart library…</div>';return}
...
const candle=points.map(p=>({time:Math.floor(p.timestamp/1000)+19800,open:p.open,high:p.high,low:p.low,close:p.close}));state.priceSeries.setData(candle);
...
state.overlays.forEach(({key,line})=>{const checkbox=$('sp3-'+key),hasData=key!=='vwap'||ind.vwap.some(v=>v!=null);if(checkbox){checkbox.disabled=!hasData;checkbox.title=hasData?'Toggle '+key.toUpperCase():'VWAP unavailable: provider did not supply traded volume';}line.applyOptions({visible:hasData&&!!checkbox?.checked});});
...
['ema9','ema21','ema50','vwap'].forEach(key=>$('sp3-'+key)?.addEventListener('change',()=>{const checkbox=$('sp3-'+key),overlay=state.overlays.find(x=>x.key===key);if(checkbox&&!checkbox.disabled&&overlay)overlay.line.applyOptions({visible:checkbox.checked});}));
...
if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',spStartAdvanced,{once:true});}else{spStartAdvanced();}
