// Market loader v20260924-r2
(() => {
  'use strict';

  // Do NOT share the old loader guard. WordPress may have an older copy running first.
  if (window.__SP_MARKET_LOADER_20260924_R2__) return;
  window.__SP_MARKET_LOADER_20260924_R2__ = true;
  ...
  function render(data){
    ...
    const sectors=i.sectors||data.sectors||{}, vals=Object.values(sectors).map(x=>num(x?.change)).filter(x=>x!=null), avg=vals.length?vals.reduce((a,b)=>a+b,0)/vals.length:null;
    set('spmi-pressure',avg==null?'N/A':avg>.25?'BUYING':avg<-.25?'SELLING':'BALANCED');
    const label=String(regime.label||'MIXED').toUpperCase(); set('spmi-final-bias',label); set('spmi-final-score',score);
  ...
  async function load(){
    try{
      ...
      set('spmi-market-status','DATA ERROR'); set('spmi-final-message','Live data error: '+e.message);
    }
  }

  clock(); load(); setInterval(clock,1000); setInterval(load,15000);
  console.log('[StrikePulse] MARKET LOADER READY v20260924-r2');
})();