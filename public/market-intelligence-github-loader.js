// Market loader v20260924 immutable CDN release
(() => {
  'use strict';
  ...
  const set = (id, value) => { const el = $(id); if (el) el.textContent = value == null || value === '' ? '—' : value; };
  const num = v => { const x = Number(v); return Number.isFinite(x) ? x : null; };
  const price = v => { const x = num(v); return x == null ? '—' : x.toLocaleString('en-IN', {minimumFractionDigits:2, maximumFractionDigits:2}); };
  const change = v => { const x = num(v); return x == null ? '—' : (x >= 0 ? '+' : '') + x.toFixed(2) + '%'; };
  ...
  function render(data) {
    ...
    const rawScore = num(regime.score), score = rawScore == null ? null : Math.round(Math.max(0, Math.min(100, 50 + rawScore / 2)));
    ...
    set('spmi-driver-strongest', drivers.strongest?.name || drivers.strongest || 'N/A'); set('spmi-driver-weakest', drivers.weakest?.name || drivers.weakest || 'N/A');
    const sectors = i.sectors || data.sectors || {}, values = Object.values(sectors).map(x => num(x?.change)).filter(x => x != null), avg = values.length ? values.reduce((a,b)=>a+b,0)/values.length : null;
    set('spmi-pressure', avg == null ? 'N/A' : avg > .25 ? 'BUYING' : avg < -.25 ? 'SELLING' : 'BALANCED');
    const finalLabel = String(regime.label || 'MIXED').toUpperCase(); set('spmi-final-bias', finalLabel); set('spmi-final-score', score);
    set('spmi-final-message', 'Market regime is ' + finalLabel.toLowerCase() + '; ' + (b ? 'breadth data available' : 'breadth data unavailable') + ', ' + (num(vix.price) == null ? 'VIX unavailable' : 'India VIX ' + price(vix.price)) + '.');
    console.log('[STRIKE PULSE] MARKET DATA UPDATED', data);
  }

  async function load() {
    try { const response = await fetch(API + '?t=' + Date.now(), {cache:'no-store'}); if (!response.ok) throw new Error('API HTTP ' + response.status); const data = await response.json(); if (!data.success) throw new Error(data.error || 'API success=false'); render(data); }
    catch (e) { console.error('[STRIKE PULSE] MARKET API ERROR:', e); set('spmi-market-status','DATA ERROR'); set('spmi-final-message','Live data error: ' + e.message); }
  }

  clock(); load(); setInterval(clock,1000); setInterval(load,15000); console.log('[StrikePulse] MARKET LOADER READY v20260924');
})();