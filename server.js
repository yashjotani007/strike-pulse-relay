...
function calculateMaxPain(rows){let bestStrike=null,bestLoss=Infinity;for(const candidate of rows){let loss=0;for(const x of rows){const callOI=x.ce?.openInterest||0;const putOI=x.pe?.openInterest||0;if(candidate.strike>x.strike)loss+=callOI*(candidate.strike-x.strike);if(candidate.strike<x.strike)loss+=putOI*(x.strike-candidate.strike);}if(loss<bestLoss){bestLoss=loss;bestStrike=candidate.strike;}}return bestStrike;}
function build(b,expiry){const z=normalize(b,expiry);if(!z.rows.length)throw Error("NIFTY option chain data not available");const spot=z.spot||price.nifty;if(!spot)throw Error("NIFTY spot unavailable");let ai=0,d=Infinity;z.rows.forEach((x,i)=>{const q=Math.abs(x.strike-spot);if(q<d){d=q;ai=i;}});const rows=z.rows.slice(Math.max(0,ai-5),ai+6).map(x=>({strike:x.strike,ce:{ltp:num(x.ce?.lastPrice),oi:num(x.ce?.openInterest),oiChange:num(x.ce?.changeinOpenInterest),volume:num(x.ce?.totalTradedVolume)},pe:{ltp:num(x.pe?.lastPrice),oi:num(x.pe?.openInterest),oiChange:num(x.pe?.changeinOpenInterest),volume:num(x.pe?.totalTradedVolume)}}));const callOI=rows.reduce((s,x)=>s+(x.ce.oi||0),0),putOI=rows.reduce((s,x)=>s+(x.pe.oi||0),0);const callOIChange=rows.reduce((s,x)=>s+(x.ce.oiChange||0),0);const putOIChange=rows.reduce((s,x)=>s+(x.pe.oiChange||0),0);const pcr=callOI?putOI/callOI:null;const maxPain=calculateMaxPain(z.rows);const result={success:true,source:"nse",spot,expiry,atmStrike:z.rows[ai].strike,atm:z.rows[ai].strike,callOI,putOI,callOIChange,putOIChange,pcr,maxPain,rows,updated:new Date().toISOString()};result.data={...result};return result;}
